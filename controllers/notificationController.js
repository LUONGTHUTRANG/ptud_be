...
import FCMService from "../utils/fcmService.js";
...
export const createNotification = async (req, res) => {
  try {
    ...
    // Handle recipients and FCM notification based on scope
    let recipients = [];
    ...
    if (target_scope === "INDIVIDUAL") {
      ...
      const tokenResults = await FCMTokenModel.getTokensByStudents(ids);
      fcmTokens = tokenResults.map((r) => r.fcm_token);
      ...
      // Get FCM tokens for all students in these rooms
      for (const roomIdItem of ids) {
        const tokens = await FCMTokenModel.getTokensByRoom(roomIdItem);
      ...
      // Get FCM tokens for all students in these buildings
      for (const buildingIdItem of ids) {
        const tokens = await FCMTokenModel.getTokensByBuilding(buildingIdItem);
        fcmTokens.push(...tokens);
      }
    } else if (target_scope === "ALL") {
      // Get all FCM tokens
      fcmTokens = await FCMTokenModel.getAllTokens();
    }
    ...
    // Send FCM notifications in background (don't block response)
    if (fcmTokens.length > 0) {
      const uniqueTokens = [...new Set(fcmTokens)]; // Remove duplicates
      FCMService.sendToTokens(uniqueTokens, notificationPayload, {
